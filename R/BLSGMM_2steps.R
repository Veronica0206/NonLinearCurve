get_BLSGMM_1st <- function(dat, nT, nClass, traj_var, t_var, res_ratio = rep(4, 2), rho = rep(0.1, 2), starts = NA, prop_starts = rep(0.5, 2),
                           loc = 1, scale = 0.25, extraTries = NA, paraNames, optimizer = "CSOLNP"){
  ...
  if (sum(prop_starts) != 1){
    ...
    if (any(is.na(starts))){
      starts <- list()
      ID <- 1:nrow(dat)
      dat_order <- dat[ID[order(apply(dat[, paste0(traj_var, 1:nT)], 1, mean))], ]
      sub_n <- floor(nrow(dat)/sum(prop_starts) * prop_starts)
      sub_start <- c(1, cumsum(sub_n[1:(nClass - 1)]) + 1)
      sub_end <- sub_start + sub_n - 1
      for (k in 1:nClass){
        subdat <- dat_order[sub_start[k]:sub_end[k], c(paste0(traj_var, 1:nT), paste0(t_var, 1:nT))]
        submodel <- getBLSGM_Fixed(dat = subdat, nT = nT, traj_var = traj_var, t_var = t_var, res_ratio = res_ratio[k], rho = rho[k],
                                   loc = loc, scale = scale, paraNames = paraFixed)[[1]]
  ...
  model_mx <- mxModel("Growth Mixture Model, BLSGM with Fixed Knots, First Step",
  ...
  return(list(model, estimate_out, est_starts))
}

get_BLSGMM_2nd <- function(dat, nT, nClass, traj_var, t_var, clus_cov, starts, beta_starts, extraTries = NA, optimizer = "CSOLNP"){
  mxOption(model = NULL, key = "Default optimizer", optimizer, reset = FALSE)
  ### Define manifest variables
  manifests <- paste0(traj_var, 1:nT)
  ### Define latent variables
  ...
  objective <- mxFitFunctionML()
  model_mx <- mxModel("Growth Mixture Model, BLSGM with Fixed Knots, Second Step",
