getPBLSGM_Random <- function(dat, nT, traj_var, t_var, gv_adjust = rep(1, 2), res_ratio = rep(4, 2), rho = rep(0.1, 2), btw_rho = rep(0.1, 1),
  ...
  if (any(is.na(starts))){
    ...
    for (traj in 1:length(traj_var)){
      BLSGM_R <- getBLSGM_Random(dat = dat, nT = nT[traj], traj_var = traj_var[traj], t_var = t_var[traj], paraNames = uni_paraNames)
      ### Mean vector (in the reparameterized framework)
      uni_mean0.s[[length(uni_mean0.s) + 1]] <- c(BLSGM_R[[1]]$mean_s$result, BLSGM_R[[1]]$mug$values)
      ### var-cov matrix (in the original framework)
      ###################
      uni_psi0[[length(uni_psi0) + 1]] <- BLSGM_R[[1]]$psi$result
      uni_grad0[[length(uni_grad0) + 1]] <- matrix(c(1, BLSGM_R[[1]]$mug$values, 0, BLSGM_R[[1]]$mean$result[2, ],
                                                     0, 0.5, 0.5, 0,
                                                     0, -0.5, 0.5, 0,
                                                     0, 0, 0, 1), nrow = 4, byrow = T)
      uni_residual[[length(uni_residual) + 1]] <- BLSGM_R[[1]]$output$estimate[1]
    }
