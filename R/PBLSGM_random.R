getPBLSGM_Random <- function(dat, nT, traj_var, t_var, gv_adjust = rep(1, 2), res_ratio = rep(4, 2), rho = rep(0.1, 2), btw_rho = rep(0.1, 1), starts = NA, extraTries = NA, paraNames, optimizer = "CSOLNP"){
  mxOption(model = NULL, key = "Default optimizer", optimizer, reset = FALSE)
  if (any(is.na(starts))){
    dat_traj <- dat_time <- list();
    eta0 <- eta1 <- eta2 <- gamma <- psi00 <- psi11 <- psi22 <- psigg <- uni_residual <- list();
    ...
    for (traj in 1:length(traj_var)){
      ...
      for (ratio in seq(2, 0, -0.05)){
        ...
        if (length(knot_candidate) == 2){
          ...
          #### Decide the initial value of the knot variance
          psigg[[length(psigg) + 1]] <- var(apply(dat_time[[traj]][, knot_candidate + 1], 1, mean, na.rm = T)) * gv_adjust[traj]
          break
        }
      }
      for (j in 1:(nT[traj] - 1)){
      ...
      uni_psi0[[length(uni_psi0) + 1]] <-
        matrix(c(psi00[[traj]], rho[traj] * sqrt(psi00[[traj]] * psi11[[traj]]), rho[traj] * sqrt(psi00[[traj]] * psi22[[traj]]), rho[traj] * sqrt(psi00[[traj]] * psigg[[traj]]),
                 rho[traj] * sqrt(psi00[[traj]] * psi11[[traj]]), psi11[[traj]], rho[traj] * sqrt(psi11[[traj]] * psi22[[traj]]), rho[traj] * sqrt(psi11[[traj]] * psigg[[traj]]),
                 rho[traj] * sqrt(psi00[[traj]] * psi22[[traj]]), rho[traj] * sqrt(psi11[[traj]] * psi22[[traj]]), psi22[[traj]], rho[traj] * sqrt(psi22[[traj]] * psigg[[traj]]),
                 rho[traj] * sqrt(psi00[[traj]] * psigg[[traj]]), rho[traj] * sqrt(psi11[[traj]] * psigg[[traj]]), rho[traj] * sqrt(psi22[[traj]] * psigg[[traj]]), psigg[[traj]]), nrow = 4)
      ...
      uni_grad0[[length(uni_grad0) + 1]] <- matrix(c(1, uni_mean0[[traj]][4], 0, uni_mean0[[traj]][2],
                                                     ...
                                                     0, -0.5, 0.5, 0,
                                                     0, 0, 0, 1), nrow = 4, byrow = T)
      uni_mean0.s[[length(uni_mean0.s) + 1]] <- c(uni_func0[[traj]][1:3, 1:3] %*% uni_mean0[[traj]][1:3], uni_mean0[[traj]][4])
    }
    ...
    for (traj in 1:(length(traj_var) - 1)){
      psi0[((traj - 1) * 4 + 1):((traj - 1) * 4 + 4), (traj * 4 + 1):(traj * 4 + 4)] <-
        matrix(c(btw_rho[traj] * sqrt(psi00[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi00[[traj]] * psi11[[traj + 1]]),
                 btw_rho[traj] * sqrt(psi00[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psi00[[traj]] * psigg[[traj + 1]]),
                 btw_rho[traj] * sqrt(psi11[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi11[[traj]] * psi11[[traj + 1]]),
                 btw_rho[traj] * sqrt(psi11[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psi11[[traj]] * psigg[[traj + 1]]),
                 btw_rho[traj] * sqrt(psi22[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi22[[traj]] * psi11[[traj + 1]]),
                 btw_rho[traj] * sqrt(psi22[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psi22[[traj]] * psigg[[traj + 1]]),
                 btw_rho[traj] * sqrt(psigg[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psigg[[traj]] * psi11[[traj + 1]]),
                 btw_rho[traj] * sqrt(psigg[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psigg[[traj]] * psigg[[traj + 1]])), nrow = 4)
      psi0[(traj * 4 + 1):(traj * 4 + 4), ((traj - 1) * 4 + 1):((traj - 1) * 4 + 4)] <-
        t(matrix(c(btw_rho[traj] * sqrt(psi00[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi00[[traj]] * psi11[[traj + 1]]),
                   btw_rho[traj] * sqrt(psi00[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psi00[[traj]] * psigg[[traj + 1]]),
                   btw_rho[traj] * sqrt(psi11[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi11[[traj]] * psi11[[traj + 1]]),
                   btw_rho[traj] * sqrt(psi11[[traj]] * psi22[[traj + 1]]), btw_rho[traj] * sqrt(psi11[[traj]] * psigg[[traj + 1]]),
                   btw_rho[traj] * sqrt(psi22[[traj]] * psi00[[traj + 1]]), btw_rho[traj] * sqrt(psi22[[traj]] * psi11[[traj + 1]]),
