getBLSGM_Random <- function(dat, T_records, traj_var, t_var, res_ratio = 4, rho = 0.1, rho_gamma = 0, starts = NA, loc = 1, scale = 0.25, extraTries = NA,
  ...
  if (any(is.na(starts))){
    ...
    reduced_model <- getBLSGM_Fixed(dat = dat, T_records = T_records, traj_var = traj_var, t_var = t_var, res_ratio = res_ratio, rho = rho, original = F)
