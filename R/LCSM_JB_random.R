getLCSM_JB_random <- function(dat, T_records, traj_var, t_var, lin_sep = 2, res_ratio = 4, starts = NA, loc = 1, scale = 0.25, extraTries = NA, original = T, paraNames = NA){
  ...
  if (any(is.na(starts))){
    ...
    psi0[1:3, 1:3] <- cov(growth_factor)
  ...
  latents <- c("eta0", "eta1", "eta2", "delta", paste0("ly", 1:nT), paste0("dy", 2:nT))
  outDef <- midTime <- outLag <- CHG_b <- CHG_i <- slp_est <- slp_se <- outLoads2 <- outLoads3 <- list()
  ...
  for (j in 1:nT){
    ...
    if (j == 1){
      ...
      outLoads3[[j]] <- mxAlgebra(0, name = paste0("L3", j))
      ...
      outLoads3[[j]] <- mxAlgebraFromString(paste0("(1 + gamma_s *  mid_t", j, ") * mueta2 * exp(gamma_s * mid_t", j, ")"),
                                            name = paste0("L3", j))
  ...
  model_mx <- mxModel("Jenss Bayley Latent Change Model, Random Ratio of Growth Acceleration", type = "RAM",
                      ...
                      mxPath(from = "delta", to = paste0("dy", 2:nT), arrows = 1, free = F, values = 0,
                             labels = paste0("L3", 2:nT, "[1,1]")),
                      ...
                      mxPath(from = latents[1:4], to = latents[1:4], arrows = 2,
                             connect = "unique.pairs", free = T,
                             values = starts[5:14],
                             labels = c("psi00", "psi01_s", "psi02", "psi0g_s",
                                        "psi11_s", "psi12_s", "psi1g_s",
                                        "psi22", "psi2g_s",
                                        "psigg_s")),
                      ...
                      mxAlgebra(psi0g_s/duration, name = "psi0g"),
                      mxAlgebra(psi11_s/duration^2, name = "psi11"),
                      mxAlgebra(psi12_s/duration, name = "psi12"),
                      mxAlgebra(psi1g_s/duration^2, name = "psi1g"),
                      mxAlgebra(psi2g_s/duration, name = "psi2g"),
                      mxAlgebra(psigg_s/duration^2, name = "psigg"),
                      mxAlgebra(rbind(mueta0, mueta1, mueta2), name = "mean0"),
                      mxAlgebra(rbind(cbind(psi00, psi01, psi02, psi0g),
                                      cbind(psi01, psi11, psi12, psi1g),
                                      cbind(psi02, psi12, psi22, psi2g),
                                      cbind(psi0g, psi1g, psi2g, psigg)), name = "psi0"),
                      outDef, midTime, outLag, outLoads2, outLoads3, slp_est, slp_se, CHG_i, CHG_b,
