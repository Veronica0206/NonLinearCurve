getPBLSGM_Fixed <- function(dat, T_records, traj_var, t_var, res_ratio = rep(4, 2), rho = rep(0.1, 2), btw_rho = rep(0.1, 1),
                            btw_res = rep(0.1, 1), starts = NA, loc = 1, scale = 0.25, extraTries = NA, original = T,
                            paraNames = NA, optimizer = "CSOLNP"){
    ...
    for (traj_i in 1:(length(traj_var) - 1)){
      for (traj_j in traj_i:(length(traj_var) - 1)){
        multi_var0[((traj_i - 1) * 3 + 1):((traj_i - 1) * 3 + 3), (traj_j * 3 + 1):(traj_j * 3 + 3)] <-
          btw_rho[traj_i] * sqrt(diag(uni_var0[[traj_i]]) %*% t(diag(uni_var0[[traj_j + 1]])))
  ...
  for (traj in 1:length(traj_var)){
    for (j in T_records[[traj]]){
      outDef[[j]] <- mxMatrix("Full", 1, 1, free = F, labels = paste0("data.", t_var[1], j),
  ...
  for (traj_i in 1:(length(traj_var) - 1)){
    for (traj_j in traj_i:(length(traj_var) - 1)){
      ...
      psi_btw_s_L[[length(psi_btw_s_L) + 1]] <- mxAlgebraFromString(paste0("rbind(cbind(psi0s0s", traj_var[traj_i], traj_var[traj_j + 1],
                                                                           ...
                                                                           ", psi2s2s", traj_var[traj_i], traj_var[2],"))"),
  ...
  if(original){
    ...
    model.est <- c(unlist(outcome_est), unlist(btw_est), model.para[grep("residuals", model.para$name), 2])
    model.se <- c(unlist(outcome_se), unlist(btw_se), model.para[grep("residuals", model.para$name), 3])
