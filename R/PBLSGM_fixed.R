getPBLSGM_Fixed <- function(dat, nT, traj_var, t_var, res_ratio = rep(4, 2), rho = rep(0.1, 2), btw_rho = rep(0.1, 1),
                            btw_res = rep(0.3, 1), starts = NA, loc = 1, scale = 0.25, extraTries = NA, original = T, paraNames = NA, optimizer = "CSOLNP"){
  ...
  if (any(is.na(starts))){
    ...
    for (traj in 1:length(traj_var)){
      BLSGM_F <- getBLSGM_Fixed(dat = dat, nT = nT[traj], traj_var = traj_var[traj], t_var = t_var[traj],
                                res_ratio = res_ratio[traj], rho = rho[traj],
      ...
      ###################
      uni_psi0[[length(uni_psi0) + 1]] <- BLSGM_F$psi$result
