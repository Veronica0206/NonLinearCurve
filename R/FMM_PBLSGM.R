getFMM_PBLSGM <- function(dat, T_records, nClass, traj_var, t_var, res_ratio = list(rep(4, 2), rep(4, 2)), rho = list(rep(0.1, 2), rep(0.1, 2)),
                          btw_rho = list(rep(0.1, 1), rep(0.1, 1)), btw_res = list(rep(0.3, 1), rep(0.3, 1)), starts = NA,
                          prop_starts = rep(0.5, 2), loc = 1, scale = 0.25, extraTries = NA, original = T, paraNames = NA, optimizer = "CSOLNP"){
  ...
  if (sum(prop_starts) != 1){
    ...
    if (any(is.na(starts))){
      ...
      for (traj in 1:length(traj_var)){
        ...
        FMM <- getFMM_BLSGM(dat = dat, T_records = T_records[[traj]], nClass = nClass, traj_var = traj_var[traj], t_var = t_var[traj], original = F)
      ...
      for (k in 1:nClass){
        ...
        submodel <- getPBLSGM_Fixed(dat = subdat, T_records = T_records, traj_var = traj_var, t_var = t_var, res_ratio = res_ratio[[k]], rho = rho[[k]],
    ...
    for (traj_i in 1:(length(traj_var) - 1)){
      for (traj_j in traj_i:(length(traj_var) - 1)){
        ...
        psi_btw_s_L[[length(psi_btw_s_L) + 1]] <- mxAlgebraFromString(paste0("rbind(cbind(c", k, "psi0s0s", traj_var[traj_i], traj_var[traj_j + 1],
                                                                             ...
                                                                             ", c", k, "psi2s2s", traj_var[traj_i], traj_var[2],"))"),
