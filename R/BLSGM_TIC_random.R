getBLSGM_TIC_Random <- function(dat, T_records, traj_var, t_var, x_var, gv_adjust = 1, res_ratio = 4, rho = 0.1, rho_gamma = 0, starts = NA,
  ...
  if (any(is.na(starts))){
    ...
    reduced_model <- getBLSGM_TIC_Fixed(dat = dat, T_records = T_records, traj_var = traj_var, t_var = t_var, x_var = x_var, res_ratio = res_ratio, rho = rho, original = F)
