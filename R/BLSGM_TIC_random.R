getBLSGM_TIC_Random <- function(dat, nT, traj_var, t_var, x_var, gv_adjust = 1, res_ratio = 4, rho = 0.1, starts = NA, loc = 1, scale = 0.25, extraTries = NA, paraNames, optimizer = "CSOLNP"){
  ...
  if (any(is.na(starts))){
    ...
    for (ratio in seq(2, 0, -0.05)){
      ...
      if (length(knot_candidate) >= 2){
        #### Decide the initial value of the knot mean & that of path coefficient(s) to the intercept
        gamma <- as.numeric(lm(knot ~ ., data = data.frame(knot = apply(dat_time[, knot_candidate + 1], 1, mean, na.rm = T), dat_covariate),
                               na.action = na.exclude)$coefficients[1])
        BetaG <- as.numeric(lm(knot ~ ., data = data.frame(cbind(knot = apply(dat_time[, knot_candidate + 1], 1, mean, na.rm = T), dat_covariate)),
                               na.action = na.exclude)$coefficients[-1]) * sqrt(gv_adjust)
        #### Decide the initial value of the knot variance
        psigg <- var(apply(dat_time[, knot_candidate + 1], 1, mean, na.rm = T)) * gv_adjust -
    ...
    psi0 <- matrix(c(psi00, rho * sqrt(psi00 * psi11), rho * sqrt(psi00 * psi22), rho * sqrt(psi00 * psigg),
                     rho * sqrt(psi00 * psi11), psi11, rho * sqrt(psi11 * psi22), rho * sqrt(psi11 * psigg),
                     rho * sqrt(psi00 * psi22), rho * sqrt(psi11 * psi22), psi22, rho * sqrt(psi22 * psigg),
                     rho * sqrt(psi00 * psigg), rho * sqrt(psi11 * psigg), rho * sqrt(psi22 * psigg), psigg), nrow = 4)
    beta0 <- matrix(c(Beta0, Beta1, Beta2, BetaG), nrow = 4, byrow = T)
