getBLSGM_TIC_Random <- function(dat, T_records, traj_var, t_var, growth_cov, gv_adjust = 1, res_ratio = 4, rho = 0.1, rho_gamma = 0, starts = NA,
  ...
  if (any(is.na(starts))){
    ...
    reduced_model <- getBLSGM_TIC_Fixed(dat = dat, T_records = T_records, traj_var = traj_var, t_var = t_var, growth_cov = growth_cov, res_ratio = res_ratio, rho = rho, original = F)
    ...
    for (j in 1:length(mean_time)){
      if (mean_time[j] <= mug & mean_time[j + 1] >= mug){
        BetaG <- as.numeric(lm(knot ~ ., data = data.frame(cbind(knot = apply(dat_time[, c(j, j + 1)], 1, mean, na.rm = T), dat_covariate)),
                               na.action = na.exclude)$coefficients[-1])
        psigg <- var(apply(dat_time[, c(j, j + 1)], 1, mean)) -
