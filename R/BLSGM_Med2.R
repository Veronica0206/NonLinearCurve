getBLSGM_Med2 <- function(dat, M_records, Y_records, X_var, M_var, Y_var, t_var, res_ratio = rep(4, 2), rho = rep(0.1, 2), btw_res = 0.1, starts = NA,
  ...
  if (any(is.na(starts))){
    ...
    M_BLSGM_F <- getBLSGM_Fixed(dat = dat, T_records = M_records, traj_var = M_var, t_var = t_var[1], res_ratio = res_ratio[1],
    ...
    Y_BLSGM_F <- getBLSGM_Fixed(dat = dat, T_records = Y_records, traj_var = Y_var, t_var = t_var[2], res_ratio = res_ratio[2],
    ...
    for (traj_j in traj_i:(length(traj_var) - 1)){
      ...
      if (setequal(substr(traj_list[[traj_i]], 2, 2), substr(traj_list[[traj_j + 1]], 2, 2))){
        residual_cor[[length(residual_cor) + 1]] <- mxPath(from = traj_list[[traj_i]], to = traj_list[[traj_j + 1]],
                                                           arrows = 2, free = T, values = btw_res * sqrt(starts[[3]][[5]] * starts[[2]][[4]]),
