getGPMM_BLSGM <- function(dat, T_records, nClass, traj_var, t_var, growth_cov, res_ratio = rep(4, 2), rho = rep(0.1, 2), starts = NA, prop_starts = rep(0.5, 2),
  ...
  if (I(original & any(is.na(paraNames)))){
    ...
    if (any(is.na(starts))){
      starts <- list()
      FMM <- getFMM_BLSGM(dat = dat, T_records = T_records, nClass = nClass, traj_var = traj_var, t_var = t_var,
      ...
      for (k in 1:nClass){
        ...
        submodel <- getBLSGM_TIC_Fixed(dat = subdat, T_records = T_records, traj_var = traj_var, t_var = t_var, x_var = growth_cov, res_ratio = res_ratio[k], rho = rho[k], original = F)
