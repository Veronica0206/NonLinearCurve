getGPMM_BLSGM <- function(dat, nT, nClass, traj_var, t_var, growth_cov, res_ratio = rep(4, 2), rho = rep(0.1, 2), starts = NA, prop_starts = rep(0.5, 2),
  ...
  if (sum(prop_starts) != 1){
    ...
    if (any(is.na(starts))){
      starts <- list()
      FMM <- getFMM_BLSGM(dat = dat, nT = nT, nClass = nClass, traj_var = traj_var, t_var = t_var,
                          res_ratio = res_ratio, rho = rho, prop_starts = prop_starts,
                          paraNames = paraFixed, loc = 1, scale = 0.05, extraTries = 9)
      model.para <- summary(FMM[[1]])$parameters[, c(1, 5, 6)]
      FMM_post <- getPosterior(model = FMM[[1]], classProbs = FMM[[1]]$weights$values, round = 4)
      dat$label <- apply(FMM_post, 1, which.max)
      for (k in 1:nClass){
        subdat <- dat[dat$label == k, ]
        submodel <- getBLSGM_TIC_Fixed(dat = subdat, nT = nT, traj_var = traj_var, t_var = t_var, x_var = growth_cov, res_ratio = res_ratio[k], rho = rho[k],
                                       loc = loc, scale = scale, extraTries = extraTries, paraNames = paraNames)[[1]]
  ...
  model_mx <- mxModel("Growth Predictor Mixture Model, BLSGM with Fixed Knots",
