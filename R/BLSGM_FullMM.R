getFullMM_BLSGM <- function(dat, T_records, nClass, traj_var, t_var, growth_cov, clus_cov, res_ratio = rep(4, 2), rho = rep(0.1, 2), starts = NA, prop_starts = rep(0.5, 2),
  ...
  if (sum(prop_starts) != 1){
    ...
    if (any(is.na(starts))){
      ...
      FMM <- getFMM_BLSGM(dat = dat, T_records = T_records, nClass = nClass, traj_var = traj_var, t_var = t_var,
                          res_ratio = res_ratio, rho = rho, prop_starts = prop_starts, original = F)
      ...
      for (k in 1:nClass){
        ...
        submodel <- getBLSGM_TIC_Fixed(dat = subdat, T_records = T_records, traj_var = traj_var, t_var = t_var, growth_cov = growth_cov, res_ratio = res_ratio[k], rho = rho[k], original = F)
