getBLSGM_Med3 <- function(dat, X_records, M_records, Y_records, X_var, M_var, Y_var, t_var, res_ratio = rep(4, 3), rho = rep(0.1, 3), btw_res = rep(0.1, 3),
  ...
  if (any(is.na(starts))){
    ...
    X_BLSGM_F <- getBLSGM_Fixed(dat = dat, T_records = X_records, traj_var = X_var, t_var = t_var[1], res_ratio = res_ratio[1],
                                rho = rho[1], original = F)
    ...
    X_psi0 <- diag(c(X_BLSGM_F$psi$result[2, 2], X_BLSGM_F$psi_s$result[1, 1], X_BLSGM_F$psi$result[3, 3]))
    X_psi0[1, 2] <- X_psi0[2, 1] <- rho[1] * sqrt(X_psi0[1, 1] * X_psi0[2, 2])
    X_psi0[1, 3] <- X_psi0[3, 1] <- X_BLSGM_F$psi$result[2, 3]
    X_psi0[2, 3] <- X_psi0[3, 2] <- rho[1] * sqrt(X_psi0[2, 2] * X_psi0[3, 3])
    ...
    M_BLSGM_F <- getBLSGM_Fixed(dat = dat, T_records = M_records, traj_var = M_var, t_var = t_var[2], res_ratio = res_ratio[2],
                                rho = rho[2], original = F)
    ...
    M_psi0 <- diag(c(M_BLSGM_F$psi$result[2, 2], M_BLSGM_F$psi_s$result[1, 1], M_BLSGM_F$psi$result[3, 3]))
    M_psi0[1, 2] <- M_psi0[2, 1] <- rho[1] * sqrt(M_psi0[1, 1] * M_psi0[2, 2])
    M_psi0[1, 3] <- M_psi0[3, 1] <- M_BLSGM_F$psi$result[2, 3]
    M_psi0[2, 3] <- M_psi0[3, 2] <- rho[1] * sqrt(M_psi0[2, 2] * M_psi0[3, 3])
    ...
    Y_BLSGM_F <- getBLSGM_Fixed(dat = dat, T_records = Y_records, traj_var = Y_var, t_var = t_var[3], res_ratio = res_ratio[3],
                                rho = rho[3], original = F)
    ...
    Y_psi0 <- diag(c(Y_BLSGM_F$psi$result[2, 2], Y_BLSGM_F$psi_s$result[1, 1], Y_BLSGM_F$psi$result[3, 3]))
    Y_psi0[1, 2] <- Y_psi0[2, 1] <- rho[1] * sqrt(Y_psi0[1, 1] * Y_psi0[2, 2])
    Y_psi0[1, 3] <- Y_psi0[3, 1] <- Y_BLSGM_F$psi$result[2, 3]
    Y_psi0[2, 3] <- Y_psi0[3, 2] <- rho[1] * sqrt(Y_psi0[2, 2] * Y_psi0[3, 3])
  ...
  model_mx <- mxModel("Mediation Process in Trivariate Nonlinear Growth Model with Fixed Knots", type = "RAM",
                      ...
                      mxPath(from = "etaY1", to = paste0(Y_var, Y_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L1", 1:length(Y_records), "Y[1,1]")),
                      ...
                      mxPath(from = "etaY2", to = paste0(Y_var, Y_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L2", 1:length(Y_records), "Y[1,1]")),
                      mxPath(from = "etaM1", to = paste0(M_var, M_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L1", 1:length(M_records), "M[1,1]")),
                      ...
                      mxPath(from = "etaM2", to = paste0(M_var, M_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L2", 1:length(M_records), "M[1,1]")),
                      mxPath(from = "etaX1", to = paste0(X_var, X_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L1", 1:length(X_records), "X[1,1]")),
                      ...
                      mxPath(from = "etaX2", to = paste0(X_var, X_records), arrows = 1, free = F, values = 0,
                             labels = paste0("L2", 1:length(X_records), "X[1,1]")),
