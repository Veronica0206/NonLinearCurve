...
PBLSGM_F <- getPBLSGM_Fixed(dat = E1_dat, nT = rep(10, 2), traj_var = c("Y", "Z"), t_var = rep("T", 2), paraNames = paraBiFixed)

PBLSGM_R <- getPBLSGM_Random(dat = E1_dat, nT = rep(10, 2), traj_var = c("Y", "Z"), t_var = rep("T", 2),  paraNames = paraBiRandom)
...
GPMM_BLSGM <- getGPMM_BLSGM(dat = E2_dat, nT = 10, nClass = 2, traj_var = "Y", t_var = "T", growth_cov = c("ex1", "ex2"),
                            paraNames = paraFixedTIC)

##### Full Mixture Model
FullMM_BLSGM <- getFullMM_BLSGM(dat = E2_dat, nT = 10, nClass = 2, traj_var = "Y", t_var = "T", growth_cov = c("ex1", "ex2"),
                                clus_cov = c("gx1", "gx2"), paraNames = paraFixedTIC)

### Test on real world data set
data("Math_dat")
Math_dat$T1 <- Math_dat$T1 - 66
Math_dat$T2 <- Math_dat$T2 - 66
Math_dat$T3 <- Math_dat$T3 - 66
Math_dat$T4 <- Math_dat$T4 - 66
Math_dat$T5 <- Math_dat$T5 - 66
Math_dat$T6 <- Math_dat$T6 - 66
Math_dat$T7 <- Math_dat$T7 - 66
Math_dat$T8 <- Math_dat$T8 - 66
Math_dat$T9 <- Math_dat$T9 - 66
...
set.seed(20181022)
BLSGM_TIC_R_Math <- getBLSGM_TIC_Random(dat = Math_dat, nT = 9, traj_var = "M", t_var = "T", x_var = c("ex1", "ex2"),
                                        paraNames = paraRandomTIC, extraTries = 29)
